...
import cn.itbaizhan.action.UploadUtil;
...
public class CommodityAction extends ActionSupport {
	...
	public String upCommodityById(){
		int commId= commodity.getCommodityId();
		System.out.println(commId);
		Commodity commodity = this.service.upCommodityById(commId);
		ActionContext.getContext().getSession().put("commodityById", commodity);
	...
	public String updateCommodity() {
		...
		if (fileName==null){
			commodity.setImage(str);
			//vo.setId(commodity.getCommodityId());
			//vo.setImageUrl(fileName);
			//this.service.setImageURL(vo);
		}else {
			try {
		...
		ActionContext.getContext().put("addComessage", commodity.getCommodityName()+"修改成功");

		return "updateCommoditySuccess";
	}
